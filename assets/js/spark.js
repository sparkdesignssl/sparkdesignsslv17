...
function initLava() {
  ...
  let lavaRaf;
  function drawLava() {
    lavaRaf = requestAnimationFrame(drawLava);
    t += 0.006;
    ...
    blobs.forEach(b => {
      b.x += b.vx + Math.sin(t + b.phase) * 0.28;
      b.y += b.vy + Math.cos(t * 0.7 + b.phase) * 0.22;
      if (b.x < -b.r) b.x = canvas.width + b.r;
      if (b.x > canvas.width + b.r) b.x = -b.r;
      if (b.y < -b.r) b.y = canvas.height + b.r;
      if (b.y > canvas.height + b.r) b.y = -b.r;

      const pulse = 0.7 + Math.sin(t * 1.2 + b.phase) * 0.3;
      const g = ctx.createRadialGradient(b.x, b.y, 0, b.x, b.y, b.r * pulse);
      g.addColorStop(0,   `hsla(${b.hue},100%,22%,0.55)`);
      g.addColorStop(0.4, `hsla(${b.hue},95%,12%,0.38)`);
      g.addColorStop(1,   `hsla(${b.hue},80%,5%,0)`);
      ctx.fillStyle = g;
      ctx.beginPath();
